 ...
 * regardless of environment. Handles every shape that field has held historically:
 ...
 *                                         -> built into "{API base}/storage/foo.webp"
 */
export const getImageUrl = (path?: string | null): string => {
  ...
  return `${API_CONFIG.BASE_URL}/storage/${path.replace(/^\/+/, '')}`;
};
