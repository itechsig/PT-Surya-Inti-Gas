...
// Type definitions (same as Product.tsx)
type Product = {
  id: string;
  title: string;
  description: string;
...
// Product data (same as Product.tsx)
...
const findProductById = (id: string): { product: Product; category: string; subCategory: string } | null => {
  for (const [mainCat, subCats] of Object.entries(productCategories)) {
    for (const [subCat, data] of Object.entries(subCats)) {
      ...
      const product = subCategoryData.products.find((p: Product) => p.id === id);
      if (product) {
...
export function ProductDetail() {
  ...
  useEffect(() => {
    ...
    if (productId) {
      const data = findProductById(productId);
      setProductData(data);
    } else {
      setProductData(null);
    }
        ...
        <section className="products-section" id="products">
          <div className="products-container">
            <div className="products-header">
              <h2 className="products-title">Memuat...</h2>
            </div>
          </div>
        </section>
      </div>
    ...
    return (
      <div className="products-corporate">
        <section className="products-section" id="products">
          <div className="products-container">
            <div className="products-header">
              <h2 className="products-title">Produk tidak ditemukan</h2>
              <button onClick={handleBack} className="products-tab" aria-label="Back to product list">
  ...
  return (
    <div className="products-corporate">
      <section className="products-section" id="products" style={{
        ...
        <div className="products-container">
          {/* Back Button */}
          <button onClick={handleBack} className="products-tab" style={{ marginBottom: '20px' }} aria-label="Back to product list">
            ← Kembali ke Daftar Produk
          </button>
